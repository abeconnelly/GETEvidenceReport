#!/usr/bin/python
# Filename: convert_from_gfg.py
"""Conversion of Genes for Good data to GFF for genome processing
...
def convert(genotype_input):
    """Take in Genes for Good genotype data, yield GFF formatted lines"""
    ...
    for line in genotype_data:
        ...
        if not header_done:
            if re.match("#", line):
                ...
                if re.search("GRCh37", line):
                    build = "b37"
                elif re.search("GRCh36", line):
                    build = "b36"
                continue
            else:
        ...
        elif re.match(r'(X|Y|MT|[1-9]|1[0-9]|2[0-2]):(\d+)[acgtACGT-]>[acgtACGT-]', data[0]):
            attributes = attributes + '; db_xref dbsnp:' + data[0]
...
def convert_to_file(genotype_input, output_file):
    """Convert a Genes for Good file and output GFF-formatted data to file"""
